...
func (m *Mutex) TryLock(ctx context.Context) (bool, error) {
	...
	if m.extCh != nil {
		go func() {
			log.Println("watch dog start:", m.name)
			extendMax, count := 3, 0
			for {
				timer := time.NewTimer(m.expire / 3 * 2)
				select {
				case <-m.extCh:
					log.Println("watch dog exit by unlock:", m.name)
					return
				case <-timer.C:
					timer.Stop()
...
// TODO 增加宿主重入方法
func (c *Client) NewMutexWithMasterExtend(name string, uniqueID string, expired time.Duration) *Mutex {
	var opts []redsync.Option
	if expired > 0 {
		opts = append(opts, redsync.WithExpiry(expired))
	} else {
	...
	if uniqueID != "" {
		opts = append(opts, redsync.WithValue(uniqueID))
	}
	m := c.rs.NewMutex(name, opts...)
	return &Mutex{
		name:   name,
